+++
title = "The Log Log: A Log About Building Logs on Logs"
date = 2026-05-01
...
The journey we'll go on here relates to the storage engine of Materialize, known as `persist`. For a bit of initial context & vocabulary: `persist` commits data by first writing the bulk of the bytes given to it into object storage (e.g. S3), and then writing pointers to that object storage into its metadata storage, known as `Consensus` (which in production is likely to be backed by Postgres or Cockroach); `persist` is designed to store [differential dataflow collections](https://github.com/TimelyDataflow/differential-dataflow), and an individual collection maps to a "shard" -- these are likely to be unfamiliar terms so mentally mapping one `persist` shard to mean the durable storage for one table in a database will get you pretty far.
